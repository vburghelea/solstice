...
<Callout>
  The `--config` flag is used to specify the path to the file where you have created your
...
<Callout>
  This is a [catch-all](https://nitro.build/guide/routing#catch-all-route) route that will
...
<Callout>
  This will enable CORS for all routes. You can customize the `origin` property to allow
  requests from specific domains. Ensure that the config is in sync with your frontend
  application.
</Callout>
