...
- Compare current working tree changes against `docs/sin-rfp/archive/completed/route-tree-implementation-plan.md`.
...
- 2025-12-27: Updated per-finding status and evidence after Streams B/C/G fixes.
...
- Status legend: **Fixed** = implemented + verified in code; **Partial** = partially addressed; **Open** = needs work.
...
1. Saved report endpoints are not scoped to organization access.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/reports/reports.queries.ts`, `src/features/reports/reports.mutations.ts`, `docs/sin-rfp/archive/streams/stream-c.md`.
...
2. SIN organization admin endpoints are missing admin enforcement (privilege escalation).
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/organizations/organizations.mutations.ts`, `src/features/organizations/organizations.queries.ts`, `docs/sin-rfp/archive/streams/stream-c.md`.
...
3. Privacy admin mutations are missing admin enforcement.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/privacy/privacy.mutations.ts`, `docs/sin-rfp/archive/streams/stream-c.md`.
...
4. Reporting queries allow unauthenticated access to sensitive org data.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/reporting/reporting.queries.ts`, `docs/sin-rfp/archive/streams/stream-c.md`.
...
1. Active org selection can be bypassed or go stale across sessions.
   ...
   - Status: **Fixed** (Stream B).
   - Evidence: `src/routes/__root.tsx`, `src/features/organizations/org-context.tsx`, `src/features/organizations/org-context-utils.ts`, `docs/sin-rfp/archive/streams/stream-b.md`.
...
2. Report export relies on headers/filters that portal UI does not set.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/reports/reports.mutations.ts`, `docs/sin-rfp/archive/streams/stream-c.md`.
...
3. Delegated analytics scope does not grant access to analytics UI.
   ...
   - Status: **Open**.
   - Evidence: `src/features/organizations/organizations.access.ts`, `src/routes/dashboard/sin/analytics.tsx`.
...
4. Tenant env resolution imports client env in server contexts.
   ...
   - Status: **Fixed** (Stream B; guarded `import.meta.env`).
   - Evidence: `src/lib/env.client.ts`, `docs/sin-rfp/archive/streams/stream-b.md`.
...
5. Global admins cannot manage orgs unless they are members.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/organizations/organizations.queries.ts`, `src/features/organizations/organizations.mutations.ts`, `src/lib/auth/guards/org-guard.ts`.
...
6. Organization lookup by ID lacks auth/membership checks.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/organizations/organizations.queries.ts`.
...
7. Notification admin mutations lack admin checks, and notification creation is unauthenticated.
   ...
   - Status: **Fixed** (Stream C).
   - Evidence: `src/features/notifications/notifications.mutations.ts`, `docs/sin-rfp/archive/streams/stream-c.md`.
...
1. Analytics feature flag coupling is stricter than the plan implies.
   ...
   - Status: **Open**.
   - Evidence: `src/features/reports/reports.queries.ts`, `src/routes/dashboard/admin/sin/analytics.tsx`.
...
2. Admin section copy references SIN even when SIN admin is disabled.
   ...
   - Status: **Open**.
   - Evidence: `src/features/layouts/admin-layout.tsx`.
...
3. New migration includes `form_submissions.import_job_id` outside the route-tree plan.
   ...
   - Evidence: `src/db/migrations/0011_curly_iron_man.sql`.
